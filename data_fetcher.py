...
def generate_training_set_one_hots_numpy(label_list, feature_size, one_hot_indexes):
    ...
    for img_name in label_list:
        one_hot = np.zeros(feature_size)
        if img_name in load_label_list(one_hot_indexes):
...
def generate_training_set_one_hot_indexes(db, score_threshold=0.15):
    one_hots_indexes = {}
    save_counter = 0
    for j, img_name in enumerate(load_label_list(db)):
        if j % 1000 == 0:
            print(j, "-", img_name)
        indexes = []
        for i, test_label in enumerate(load_label_list(db)):
            score = calculate_score(img_name, test_label, db)
            if score > score_threshold:
                indexes.append(i)
        one_hots_indexes[img_name] = indexes
        if j+1 % 10000 == 0:
            with open('training_one_hot_indexes-' + str(save_counter) + '.pickle', 'wb') as handle:
